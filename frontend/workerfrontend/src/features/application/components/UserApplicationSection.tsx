...
export default function UserApplicationSection({ task, onFeedback }: UserApplicationSectionProps) {
  const { isAuthenticated, loginWithRedirect, user } = useAuth0();
  const [showApplicationForm, setShowApplicationForm] = useState(false);
  const [formMode, setFormMode] = useState<"create" | "edit">("create");

  const { data: application, isLoading: applicationLoading } = useGetApplication(task.id);
  ...
  const applyButtonBusy = applicationLoading || createApplication.isPending || updateApplication.isPending;
  ...
  const handleApplicationSuccess = (mode: "create" | "edit") => {
    ...
    onFeedback(
      "success",
      mode === "edit" ? "Hakemus päivitettiin onnistuneesti!" : "Hakemus lähetettiin onnistuneesti!",
    );
  };
  ...
  const handleDeleteApplication = async () => {
    ...
    const confirmed = window.confirm("Haluatko varmasti perua tämän hakemuksen?");
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Hakemuksen peruminen epäonnistui.";
