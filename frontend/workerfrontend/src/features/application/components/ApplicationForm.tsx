...
import type { ApplicationFormValues, ApplicationFormMode } from "../types";
...
export default function ApplicationForm({
    ...
    const defaultValues = useMemo(
        () => ({
            priceSuggestion: initialValues ? String(initialValues.priceSuggestion) : "",
            timeSuggestion: initialValues?.timeSuggestion ? toDateTimeLocal(initialValues.timeSuggestion) : "",
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                ...
                setSubmitError(error instanceof Error ? error.message : "Hakemuksen lähetys epäonnistui");
                throw error;
            }
        },
    });
        ...
        <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-6">
            ...
            >
                ...
                <form.Field
                    ...
                    validators={{
                        onChange: ({ value }) => {
                            ...
                            if (Number(value) <= 0) return "Hinnan täytyy olla suurempi kuin 0";
                            return undefined;
                        },
                    }}
                >
                    {(field) => (
                        <div>
                            <label htmlFor="priceSuggestion" className="block text-sm font-medium text-gray-700 mb-2">
                                Hintatarjous (€) *
                            </label>
                            <input
                                type="number"
                            ...
                            {field.state.meta.errors.length > 0 && (
                                <p className="text-xs text-red-600 mt-1">{field.state.meta.errors.join(", ")}</p>
                            )}
                            <p className="text-xs text-gray-500 mt-1">Työn hintatarjous: {taskPrice} €</p>
                        </div>
                    )}
                </form.Field>
                ...
                <form.Field
                    name="timeSuggestion"
                    validators={{
                        onChange: ({ value }) => (!value ? "Aikaehdotus vaaditaan" : undefined),
                    }}
                >
                    {(field) => (
                        <div>
                            <label htmlFor="timeSuggestion" className="block text-sm font-medium text-gray-700 mb-2">
                                Aikaehdotus *
                            </label>
                            <input
                                type="datetime-local"
                            ...
                            <p className="text-xs text-gray-500 mt-1">Milloin voisit tehdä työn?</p>
                        </div>
                    )}
                </form.Field>
