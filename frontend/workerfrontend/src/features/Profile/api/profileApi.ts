...
//POST METHODS
export async function createUser(getAccessToken: () => Promise<string>, userData: UserDto): Promise<UserDto> {
  const token = await getAccessToken();
...
//PUT METHODS
export async function updateUser(getAccessToken: () => Promise<string>, userData: Partial<UserDto>): Promise<UserDto> {
  const token = await getAccessToken();
