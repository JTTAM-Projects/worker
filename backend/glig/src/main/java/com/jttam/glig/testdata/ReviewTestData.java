...
public class ReviewTestData {
    ...
    public Map<String, Review> createTestReviews(Map<String, User> users, Map<String, Task> tasks,
        ...
        // Get tasks owned by different users - need enough for 18 unique review scenarios
        ...
        if (user1Tasks.size() >= 7 && user2Tasks.size() >= 8 && user3Tasks.size() >= 8 && user4Tasks.size() >= 3) {
            // User1 TASKER PROFILE (11 reviews - more than 10 to test scrolling): User1 worked as tasker
            // Tasks owned by user2/3/4, they review user1's tasker performance
            Review r1 = new Review(user2Tasks.get(0), user2, user1, 5, "Excellent work! Very professional and efficient. Would hire again.");
            Review r2 = new Review(user2Tasks.get(1), user2, user1, 3, "Decent work but could have been more thorough. Average experience.");
            Review r3 = new Review(user2Tasks.get(2), user2, user1, 3, "Acceptable work. Met basic requirements but nothing exceptional.");
            Review r4 = new Review(user2Tasks.get(6), user2, user1, 5, "Amazing attention to detail! Exceeded all expectations.");
            Review r5 = new Review(user2Tasks.get(7), user2, user1, 4, "Good service, would hire again for future projects.");
            Review r6 = new Review(user3Tasks.get(0), user3, user1, 4, "Good job overall. Completed on time and met expectations.");
            Review r7 = new Review(user3Tasks.get(1), user3, user1, 5, "Superb craftsmanship! Every detail was perfect.");
            Review r8 = new Review(user3Tasks.get(5), user3, user1, 4, "Professional and courteous. Highly skilled at their work.");
            Review r9 = new Review(user3Tasks.get(6), user3, user1, 5, "Best tasker I've worked with! Will definitely hire again.");
            Review r10 = new Review(user4Tasks.get(0), user4, user1, 5, "Outstanding service! Went above and beyond. Highly recommended!");
            Review r11 = new Review(user4Tasks.get(1), user4, user1, 4, "Reliable and competent. Would use services again.");
            
            // User1 EMPLOYER PROFILE (5 reviews): user1 owned tasks, taskers rate user1 as employer
            // Tasks owned by user1, taskers (user2/3/4) review user1's employer quality
            Review r12 = new Review(user1Tasks.get(0), user2, user1, 5, "Great employer! Clear instructions and prompt payment.");
            Review r13 = new Review(user1Tasks.get(1), user2, user1, 3, "Communication could be better, but payment was on time.");
            Review r14 = new Review(user1Tasks.get(5), user2, user1, 4, "Fair employer with reasonable expectations.");
            Review r15 = new Review(user1Tasks.get(2), user3, user1, 4, "Pleasant to work with. Good communication throughout the project.");
            Review r16 = new Review(user1Tasks.get(3), user4, user1, 5, "Perfect client! Respectful and fair. Highly recommend.");
            
            // User2 TASKER PROFILE (4 reviews): user2 worked as tasker, employers rate user2
            // Tasks owned by user1/3, they review user2's tasker performance
            Review r17 = new Review(user1Tasks.get(4), user1, user2, 5, "Fantastic work ethic! Delivered high quality results.");
            Review r18 = new Review(user3Tasks.get(2), user3, user2, 3, "Adequate performance. Some minor issues but overall okay.");
            Review r19 = new Review(user3Tasks.get(3), user3, user2, 4, "Very reliable and punctual. Would recommend.");
            Review r20 = new Review(user3Tasks.get(4), user3, user2, 5, "Exceptional service! Professional and friendly.");
            
            // User2 EMPLOYER PROFILE (3 reviews): user2 owned tasks, taskers rate user2 as employer
            // Tasks owned by user2, taskers (user3/4) review user2's employer quality
            Review r21 = new Review(user2Tasks.get(3), user3, user2, 4, "Good employer. Fair pricing and reasonable expectations.");
            Review r22 = new Review(user2Tasks.get(4), user4, user2, 5, "Excellent communication! Very satisfied with the collaboration.");
            Review r23 = new Review(user2Tasks.get(5), user3, user2, 5, "One of the best employers to work with!");
            
            // User3 TASKER PROFILE (2 reviews): user3 worked as tasker, employers rate user3
            // Tasks owned by user1/4, they review user3's tasker performance
            Review r24 = new Review(user1Tasks.get(6), user1, user3, 4, "Solid performance. Met all requirements efficiently.");
            Review r25 = new Review(user4Tasks.get(2), user4, user3, 5, "Great job! Would definitely work together again.");
