...
export const Route = createFileRoute("/_authenticated/worker/my-applications/$taskId/task-details")({
  loader: ({ context: { queryClient }, params: { taskId } }) => {
    return queryClient.ensureQueryData(taskQueries.detail(taskId));
  },
  component: TaskApplicationDetailsPage,
});

function TaskApplicationDetailsPage() {
    ...
    <div className="container mx-auto px-6 py-8">
      ...
      <div className="flex mb-6 justify-center">
        <button
          ...
          onClick={() => navigate({ to: "/worker/my-applications/$taskId/task-details", params: { taskId } })}
        >
          Tehtävän tiedot
        </button>
        <button
          className={"py-2 px-4 text-sm font-medium text-gray-500 hover:text-gray-700"}
          onClick={() => navigate({ to: "/worker/my-applications/$taskId/application-details", params: { taskId } })}
        >
          Hakemuksen tiedot
        </button>
      </div>
